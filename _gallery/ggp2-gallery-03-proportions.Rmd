...
subtitle: "Hierarchies/part-to-whole relationships: Proportions"
...
> "***In general, pie charts work well when the goal is to emphasize simple fractions, such as one-half, one-third, or one-quarter. They also work well when we have very small datasets.***" - Claus O. Wilke, Fundamentals of Data Visualization (2019)
...
```{r plot-ggpie, ref.label='movies_mpaa_avg_rating_ggpie', echo=FALSE, fig.align='center', out.width='80%', out.height='80%'}
```
...
```{r plot-stacked_density, ref.label='stacked_density', echo=FALSE, fig.align='center', out.width='90%', out.height='90%'}
```
...
> *Waffle charts display how proportions of different categorical levels make up the values in an entire variable. The counts for each level are divided by color into a square or grid display.*
...
> *Waffle charts require a special data tranformation with `ggwaffle::waffle_iron()`. Set the `group` argument in `aes_d()` as the categorical variable you want to see the relative counts for.*
...
```{r plot-waffle_plot, ref.label='waffle_plot', echo=FALSE, fig.align='center', out.width='90%', out.height='90%'}
```
...
> *We'll move the legend to the top of the plot with `ggplot2::theme(legend.position = "top")`.*
...
```{r plot-mosaic_plot, ref.label='mosaic_plot', echo=FALSE, fig.align='center', out.width='65%', out.height='65%', fig.height=4.7, fig.width=4.7}
```
...
> *We can guild treemaps in ggplot2 with the [`treemapify` package](http://wilkox.org/treemapify/)*
...
treemap_penguins_grouped <- group_by(treemap_penguins, 
                                     species, island, sex)
treemap_penguins_counts <- ungroup(count(treemap_penguins_grouped, 
                                         species, island))
```
...
```{r plot-geom_treemap, ref.label='geom_treemap', echo=FALSE, fig.align='center', out.width='90%', out.height='90%'}
```
...
```{r plot-geom_treemap_subgroup_border, ref.label='geom_treemap_subgroup_border', echo=FALSE, fig.align='center', out.width='90%', out.height='90%'}
```
...
```{r plot-geom_treemap_subgroup_text, ref.label='geom_treemap_subgroup_text', echo=FALSE, fig.align='center', out.width='90%', out.height='90%'}
```
...
```{r plot-geom_treemap_text, ref.label='geom_treemap_text', echo=FALSE, fig.align='center', out.width='90%', out.height='90%'}
```
